...
#include "JContainers/JContainers/src/jcontainers_constants.h"
...
namespace jc::api
{
	...
	{
		if (!initApi(a_root)) {
			util::report_and_fail("JContainers initialization error, see log for details");
		}
	...
	void onPostLoad()
	{
		auto result = SKSE::GetMessagingInterface()->RegisterListener(SKSE::GetPluginHandle(), JC_PLUGIN_NAME, [](SKSEMessagingInterface::Message* a_msg) {
	...
	void init()
	{
		SKSE::GetMessagingInterface()->RegisterListener(SKSE::GetPluginHandle(), "SKSE", [](SKSEMessagingInterface::Message* a_msg) {
			if (a_msg && a_msg->type == SKSEMessagingInterface::kMessage_PostLoad) {
				onPostLoad();
			}
		});
	}
