...
namespace jc::JDB
{
	...
	float solveFlt(void* a_domain, BSFixedString a_path, float a_default)
	{
		return ApiStorage::get().JDB_solveFlt(a_domain, a_path, a_default);
	}

	std::int32_t solveInt(void* a_domain, BSFixedString a_path, std::int32_t a_default)
	{
	...
	std::int32_t solveObj(void* a_domain, BSFixedString a_path, std::int32_t a_default)
	{
		return ApiStorage::get().JDB_solveObj(a_domain, a_path, a_default);
	}

	TESForm* solveForm(void* a_domain, BSFixedString a_path, TESForm* a_default)
	{
		return ApiStorage::get().JDB_solveForm(a_domain, a_path, a_default);
	}

	bool solveFltSetter(void* a_domain, BSFixedString a_path, float a_value, bool a_createMissingKeys)
	{
		return ApiStorage::get().JDB_solveFltSetter(a_domain, a_path, a_value, a_createMissingKeys);
	}

	bool solveIntSetter(void* a_domain, BSFixedString a_path, std::int32_t a_value, bool a_createMissingKeys)
	{
		return ApiStorage::get().JDB_solveIntSetter(a_domain, a_path, a_value, a_createMissingKeys);
	}

	bool solveStrSetter(void* a_domain, BSFixedString a_path, BSFixedString a_value, bool a_createMissingKeys)
	{
		return ApiStorage::get().JDB_solveStrSetter(a_domain, a_path, a_value, a_createMissingKeys);
	}

	bool solveObjSetter(void* a_domain, BSFixedString a_path, std::int32_t a_value, bool a_createMissingKeys)
	{
		return ApiStorage::get().JDB_solveObjSetter(a_domain, a_path, a_value, a_createMissingKeys);
	}
	bool solveFormSetter(void* a_domain, BSFixedString a_path, TESForm* a_value, bool a_createMissingKeys)
	{
