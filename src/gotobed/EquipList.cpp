	...
	EquipList::EquipList(std::int32_t a_obj)
	{
		if (a_obj) {
			...
			for (int i = 0; i < count; ++i) {
				...
				entries.push_back({ static_cast<TESBoundObject*>(item), static_cast<std::uint32_t>(count), slot && slot->formType == kFormType_EquipSlot ? static_cast<BGSEquipSlot*>(slot) : nullptr });
			}
		}
	}
